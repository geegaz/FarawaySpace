...
export(Vector2) var distance: Vector2 = Vector2(5, 5)
export(float, 0, 1) var amount: float = 0.5
export(float, 0, 1) var speed_distance: float = 1
export(float, 0, 1) var speed_amount: float = 1
...
func _physics_process(delta: float) -> void:
	...
	var ray: Vector3 = down * distance.y + forward * distance.x * speed_distance
	var ray_length: float = ray.length()
	
	var target_dir: Vector3 = Vector3.ZERO
	for i in directions:
		var dir: Vector3 = ray.rotated(forward, step * i)
		...
		if not collision.empty():
			...
			# Control the amount of correction
			var distance_amount: float = 1 - global_transform.origin.distance_to(collision.position) / ray_length
	...
	if target_dir != Vector3.ZERO and parent:
		target_dir = forward.linear_interpolate(target_dir.normalized(), 1 - exp(-smoothing_speed * speed_amount * amount * delta))
