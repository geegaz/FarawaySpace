...
func add_shake(node: Node, property: String, offset: Vector2, time: float, speed: float):
	...
	processes[id].shake = min(processes[id].shake + time, 1.0)
