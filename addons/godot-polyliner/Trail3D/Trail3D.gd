...
@export var sampling_mode: SamplingMode = SamplingMode.Idle: set = set_sampling_mode
@export var max_points = 10: set = set_max_points
...
func _process(delta):
	...
	if fmod(Engine.get_process_frames(),max(skip_frames+1,1)) < 0.001:
