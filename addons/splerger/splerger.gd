...
func merge_suitable_meshes_across_branches(var root : Spatial, var use_local_space : bool = false, var delete_originals : bool = true):
	var master_list = []
	...
	for n in range (sub_list.size()):
		...
		if (sl.size() > 1):
			var new_mi : MeshInstance = merge_meshinstances(sl, root, use_local_space, delete_originals)
